...
Usage:
  runSim_litt.jl [--threshold=<alpha>] [--population=<N>] [--generations=<L>] [--rate=<P>] [--monitor=<step>] [--output=<file>]
  runSim_litt.jl -h | --help
  runSim_litt.jl --version
...
# NB: pulling down ARID1A only capture the BAF complex
...
# ARID1A should not be the last subunit in the
...
compDictSimi = Dict(map(reverse, unitDict)["SMARCA4"] => 1,
...
# Ensure connectivity
map(randomCompetitionGraph!, pop)
