#=====
Testing the performance of the genetic algorithm on known structures
=====#

doc = """Run genetic algorithm to retrieve randomly generated complex structures.

Usage:
  algorithmConvergence.jl [--population=<N>] [--generations=<L>] [--rate=<P>] [--monitor=<step>] [--output=<file>]
  algorithmConvergence.jl -h | --help
  algorithmConvergence.jl --version
...
studyARIDko = String["ACTB", "ARID1B", "ARID2", "BCL11A", "BCL11B", "BCL7A", "BCL7B", "BRD7", "BRD9", "DPF1", 
    "DPF2", "DPF3", "PBRM1", "PHF10", "SMARCA2", "SMARCA4", "SMARCC1", "SMARCC2", "SMARCD1", "SMARCD2", "SMARCD3"]
studyARIDpd = String["ACTL6A", "ARID1A", "BCL7A", "BCL7B", "BCL7C", "DPF1", "DPF2", "DPF3", "SMARCA2", "SMARCA4",
    "SMARCB1", "SMARCC1", "SMARCC2", "SMARCD1", "SMARCD2", "SMARCD3", "SMARCE1", "SS18", "SS18L1"]
studyBRGko = String["ACTB", "ARID1A", "ARID1B", "ARID2", "BCL11A", "BCL11B", "BCL7A", "BCL7B", "BRD7", "BRD9", 
    "DPF1", "DPF2", "DPF3", "PBRM1", "PHF10", "SMARCA2", "SMARCC1", "SMARCC2", "SMARCD1", "SMARCD2", "SMARCD3"]
studyBRGpd = String["ACTL6A", "ACTL6B", "ARID1A", "ARID1B", "ARID2", "BCL7A", "BCL7B", "BCL7C", "BRD7", "BRD9", 
    "DPF1", "DPF2", "DPF3", "PBRM1", "PHF10", "SMARCA4", "SMARCB1", "SMARCC1", "SMARCC2", "SMARCD1", "SMARCD2", 
    "SMARCD3", "SMARCE1", "SS18", "SS18L1"]
...
structIni = structureGraph(
        Graph(M, rand(1:maxEdges)),
        copy(unitDictString),
        Dict())

# Ensure connectivity
randomCompetitionGraph!(structIni)
connectGraph!(structIni)

pdIni = structureToPulldowns(structIni)

edgeTypesARID = Dict(k => pdIni[1].edges for k in [0.1, 0.05, 0.01, 0.005])
edgeTypesBRG = Dict(k => pdIni[2].edges for k in [0.1, 0.05, 0.01, 0.005])
...
jc, ib = findmax(map(x -> observedLoss(x)[1], pop))

# 1: number of edges 
# 2: number of competition classes
# 3: final max fitness
# 4: max jaccard edges
# 5: max jaccard competition
# 6: number of edges best simulation
# 7: proportion of TP edges
# 8: number of competition classes best simulation
# 9: proportion of TP competitions
summaryRun = [ne(structIni.graph),
    length(unique(values(structIni.competition))),
    jc,
    length(intersect(edges(pop[ib].graph), edges(structIni.graph)))/length(union(edges(pop[ib].graph), 
            edges(structIni.graph))), length(intersect(getCompetitionDict(pop[ib].competition), 
        getCompetitionDict(structIni.competition)))/length(union(getCompetitionDict(pop[ib].competition), 
        getCompetitionDict(structIni.competition))),
    ne(pop[ib].graph),
    length(intersect(edges(pop[ib].graph), edges(structIni.graph)))/ne(pop[ib].graph),
    length(getCompetitionDict(pop[ib].competition)),
    length(intersect(getCompetitionDict(pop[ib].competition), 
        getCompetitionDict(structIni.competition)))/M]

save(outputFileName*".jld","summaryList",summaryRun)