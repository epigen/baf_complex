...
Usage:
  pbaf_runSim_simi.jl [--threshold=<alpha>] [--population=<N>] [--generations=<L>] [--rate=<P>] [--monitor=<step>] [--output=<file>]
  pbaf_runSim_simi.jl -h | --help
  pbaf_runSim_simi.jl --version
...
compDictSimi = Dict(map(reverse, unitDict)["SMARCA4"] => 1,
    ...
    map(reverse, unitDict)["ACTL6A"] => 9,
    map(reverse, unitDict)["ACTL6B"] => 9
)
